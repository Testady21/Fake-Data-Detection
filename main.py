...
def build_loaders(clip_features,labels,args,mode):
    # Added alignment fix for dataset size mismatch
    min_len = min(clip_features.shape[0], len(labels))
...
####################################################################################
    ...
    # build data Loaders
    print("Train features shape:", train_clip_features.shape)
...
############################# MAIN LOOP############################################
    ...
    model.eval()
    with torch.no_grad():
        test_loss,test_Acc,test_predictions,testlabels,testtargets = valid_epoch(model, test_loader,lossfunction,args)
